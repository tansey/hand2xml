...
";
        ...
        @"([a-zA-Z0-9\s]*)\s(wins|ties for)\s(the\s(high\s|low\s)?(main\s|side\s)?pot|(high\s|low\s)?side\spot\s#([0-9]+))\s\(\$?((,?[0-9]?[0-9]?[0-9])+(\.\d\d)?)\)",
            ...
            Regex actionExp =
            new Regex(
        FullTiltParser.NAME_EXPRESSION + @"\s(posts(\s(the|a)\s(dead\s)?(small|big)\sblind\sof)?|antes|checks|bets|calls|raises\sto|folds)\s?" + FullTiltParser.CHIPS_EXPRESSION + @"?(,\sand\sis\sall\sin)?",
            RegexOptions.Compiled);

            for (Match m = actionExp.Match(
        @"bl_ad raises to $42,267, and is all in"
