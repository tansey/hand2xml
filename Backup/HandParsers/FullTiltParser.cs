...
namespace HandParsers
{
    ...
    public class FullTiltParser : IHandParser
    {
        ...
        Regex actionExp =
            new Regex(
        @"([a-zA-Z0-9\s]*)\s(posts(\sthe\s(dead\s)?(small|big)\sblind\sof)?|antes|checks|bets|calls|raises\sto|folds)\s?\$?((,?[0-9]?[0-9]?[0-9])+(\.\d\d)?)?(,\sand\sis\sall\sin)?",
            RegexOptions.Compiled);

        Regex uncalledExp =
            new Regex(
        @"Uncalled\sbet\sof\s\$?((,?[0-9]?[0-9]?[0-9])+(\.\d\d)?)\sreturned\sto\s([a-zA-Z0-9\s]*)",
            RegexOptions.Compiled);
        ...
        private PokerHand parseHand(List<string> lines, string handText)
        {
            ...
            for (; !lines[curLine].StartsWith("The button is in seat #"); curLine++)
            {
                ...
                for (Match m = actionExp.Match(lines[curLine]); m.Success; m = m.NextMatch())
                {
                    ...
                    else if (gc[5].Value == "small")
                        blind.Type = gc[4].Value == "dead" ? BlindType.DeadBlind : BlindType.SmallBlind;
                    else if (gc[5].Value == "big")
                        blind.Type = gc[4].Value == "dead" ? BlindType.DeadBlind : BlindType.BigBlind;
                    else if (gc[2].Value == "posts")
                        blind.Type = BlindType.LateBlind;

                    blind.Amount = Decimal.Parse(gc[6].Value);
                ...
                if ( m.Success )
                {
                    ...
                    switch (gc[2].Value)
                    {
                        ...
                        case "calls": a.Type = ActionType.Call;
                                      a.Amount = Decimal.Parse(gc[6].Value);
                                      a.AllIn = gc[9].Value.Length == 15;
                            break;
